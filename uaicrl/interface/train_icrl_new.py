...
from common.cns_visualization import constraint_visualization_1d, constraint_visualization_2d_new, traj_visualization_2d_new, \
    traj_visualization_1d, beta_parameters_visualization, psat
...
def start_logging(filename, append=True):
...
def train(config):
    ...
    start_logging("%slog.txt" % save_model_mother_dir)
    ...
    ture_cost_function = get_true_cost_function(env_id=config['env']['train_env_id'],
                                                env_configs=env_configs)
    constraint_visualization_2d_new(env_id=config['env']['train_env_id'],
                                cost_function=ture_cost_function,
                                obs_dim=train_env.observation_space.shape[0],
    ...
    for itr in range(config['running']['n_iters']):
        ...
        if itr % config['running']['save_every'] == 0:
            traj_visualization_2d_new(env_id=config['env']['train_env_id'],
                                  config=config,
        ...
        mean_reward, std_reward, mean_nc_reward, std_nc_reward, costs = \
        ...
        if itr % config['running']['save_every'] == 0:
            ...
            constraint_visualization_2d_new(env_id=config['env']['train_env_id'],
                                        ...
                                        title='Iteration-{0}'.format(itr),
                                        force_mode='mean',
                                        gt_cost_function=ture_cost_function,
                                        )
                # if 'WGW' in config['env']['train_env_id'] and itr % config['running']['save_every'] == 0:
    ...
    constraint_visualization_2d_new(env_id=config['env']['train_env_id'],
                            ...
                            obs_dim=train_env.observation_space.shape[0],
                            acs_dim=1 if is_discrete else train_env.action_space.shape[0],
                            save_path="%s" % save_model_mother_dir,
                            model_name=args.config_file.split('/')[-1].split('.')[0],
                            title='Final-constraint',
                            force_mode='mean',
                            gt_cost_function=ture_cost_function,
                            )
    ...
    psat(
        ...
        cost_function=constraint_net.cost_function,
        plot=True,
        save_to="%shistogram.png" % save_model_mother_dir,
    )
