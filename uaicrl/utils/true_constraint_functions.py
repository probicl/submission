...
def get_true_cost_function(env_id, env_configs={}):
    ...
    if env_id == 'CustomCartPole' or env_id == 'CartPole-v0':
        return partial(pole_outside, -1, 1)
    elif env_id == 'Gridworld':
        return partial(in_unsafe_region, [(ui, uj) for ui in [3] for uj in [0, 1, 2, 3]])
    elif env_id in ['AntWall', 'AntWall-v0', 'HalfCheetahWithPos', 'HalfCheetahWithPos-v0']:
...
def pole_outside(pos_a, pos_b, obs, acs):
    # print(obs.shape, acs.shape)
    return ((obs[..., 0] < pos_a) | (obs[..., 0] > pos_b))

def in_unsafe_region(u, obs, acs):
    ret = None
    for (ui, uj) in u:
        if ret is None:
            ret = (obs[..., 0] == ui) & (obs[..., 1] == uj)
        else:
            ret |= (obs[..., 0] == ui) & (obs[..., 1] == uj)
    return ret
