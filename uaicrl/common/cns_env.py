    ...
    def _init():
        ...
        if env_id == 'CustomCartPole':
            ...
            class A:
                pass
            env.spec = A()
            env.spec.id = 'AntWall-v0'
        elif env_id == 'HalfCheetahWithPos':
            env = tools.environments.GymEnvironment(
                'HalfCheetahWithPos',
            )
            env = tools.environments.TimeLimit(env, 500)
            ...
            class A:
                pass
            env.spec = A()
            env.spec.id = 'HalfCheetahWithPos-v0'
        else:
        ...
        if is_mujoco(env_id) or env_id in ['CustomCartPole', 'Gridworld'] or env_id in ['AntWall', 'HalfCheetahWithPos']:
            print("Using external signal for env: {0}.".format(env_id), flush=True)
