...
def create_env():
    if basic.env_name == 'gridworld':
        ...
        basic.replay_buffer_size = 50000
        basic.use_gae = True
...
def make_nn():
    ...
    #     ).to(basic.device)
    #     value_nn = torch.jit.script(value_nn)
    #     print("cleanrl value_nn")
    # else:
    value_nn = torch.nn.Sequential(
...
def play_episode(env, policy_nn, constraint_fn, get_r=False):
    S, A, R, C = [], [], [], []
    S += [env.reset()]
    done = False
...
def ppo_penalty(n_epochs, env, policy_nn, value_nn, constraint_fn, additional_fn_condition=None, additional_fn_epoch_end=None, p=False):
    ...
    basic.Epoch_info = []
    best_R = -float('inf')
    best_policy_params = policy_nn.state_dict()
...
def ppo_penalty_gae(n_epochs, env, policy_nn, value_nn, constraint_fn, additional_fn_condition=None, additional_fn_epoch_end=None, p=False):
    ...
    for epoch in range(n_epochs):
        ...
        if basic.show_window_stats:
            print("Epoch %d:\tG_avg = %.2f\tGc_avg = %.2f\tMaxCostReached = %.2f\tG_window = %.2f\tMCR_window = %.2f"
                % (epoch, np.mean(G0_e_reported), np.mean(Gc0_e), float(max_cost_reached / max_cost_reached_n), np.mean(G0_all[-basic.window_size:]), np.mean(MCR_all[-basic.window_size:])))
        else:
            print("Epoch %d:\tG_avg = %.2f\tGc_avg = %.2f\tMaxCostReached = %.2f"
                % (epoch, np.mean(G0_e_reported), np.mean(Gc0_e), float(max_cost_reached / max_cost_reached_n)))
        if basic.use_early_stopping:
            if p == True:
                if np.mean(G0_e_reported) >= best_R and np.mean([float(item < basic.beta) for item in Gc0_e]) >= basic.delta:
                    old_R = copy.copy(best_R)
        ...
        feasibility_opt = torch.optim.Adam(policy_nn.parameters(), lr=basic.learning_rate_feasibility) # eps Hack
        # Penalty update on complete trajectories
        if p:
            Gc0s = []
            loss1_terms = []
        ...
        for subepoch in range(basic.ppo_subepochs):
            ...
            value_loss.backward()
            if 'mujoco' in basic.env_name or basic.env_name in ['highd']:
                torch.nn.utils.clip_grad_norm_(value_nn.parameters(), 0.5)
            value_opt.step()
            policy_opt.zero_grad()
            ## advantages = G - value_nn(S)
            advantages = Adv
            ...
            if 'mujoco' in basic.env_name or basic.env_name in ['highd']:
                torch.nn.utils.clip_grad_norm_(policy_nn.parameters(), 0.5) # cleanrl gradient clipping
            policy_opt.step()
            # combined_opt.step()
        basic.Epoch_info[-1]["policy_losses"] = policy_losses
        # print("PPO trained")
