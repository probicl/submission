...
class PPOLag:
    ...
    def train(self, evaluator=None, no_mix=False, **kwargs):
        ...
        for t in range(self.config["local_steps_per_epoch"]):
            ...
            # print(c, o[0], o[1], a)
            # print(basic.constraint_nn(torch.tensor(o, device=basic.device, dtype=torch.float)))
            c = float(c)
            # print(c, o, a)
        ...
        metrics["avg_env_ecv"] = np.mean(metrics["avg_env_ecv"])
        ...
        if kwargs["forward_only"]:
            return metrics
        new_metrics = self.update(metrics["avg_env_ecv"], metrics["avg_env_ep_len"])
