...
class Plot2D:
    ...
    def polygon2(self, brown_id, bbox_dict, loc=None, o=None, **kwargs):
        self.empty = False
        axobj = self.getax(loc=loc)
        if o is None:
            o = {}
        keys_not_used = set(o.keys())-set(bbox_dict.keys())
        for k, v in bbox_dict.items():
            if k in o.keys():
                o[k].set_xy(v)
            else:
                orig_fc = kwargs["facecolor"]
                if k == brown_id:
                    kwargs["facecolor"] = "brown"
                o[k] = Polygon(v, **kwargs)
                kwargs["facecolor"] = orig_fc
                axobj.add_patch(o[k])
        for k in keys_not_used:
            o[k].remove()
            del o[k]
        return o
